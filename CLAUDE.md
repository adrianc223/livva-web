...
- **Vercel**: project `adrianc223s-projects/livva-web`, connected to the GitHub repo (`vercel git connect`) — every push to `main` auto-deploys to production, same workflow as the main app. Live at **https://livva-web.vercel.app** (no custom domain yet — see `docs/ROADMAP.md`'s "Custom domain" section for what changes when one is bought; don't hardcode the `*.vercel.app` URL anywhere new — it already lives in exactly one place, `src/lib/siteUrl.ts`).
- **Env vars** (set via `vercel env add`, live in Vercel's project settings, never committed): `RESEND_API_KEY`, `EMAIL_FROM`, `LEAD_NOTIFY_EMAIL` set on both Production and Preview; `APP_URL` set on Production only (a Preview deployment's URL is unique per-deploy, so there's no single correct value to give it there — it falls back to `src/lib/siteUrl.ts`'s hardcoded default instead, which only affects a Preview's metadata, not its actual reachability).
...
- **Accessibility (WCAG)** — checked directly against this codebase, not assumed:
  ...
  - Every `<img>` (all via `Logo.tsx`) carries `alt="Livva"` — verified with a grep for `<img` lacking `alt=` (zero hits).
...
- **Core Web Vitals**: no heavy client-side data fetching (no DB, one small `fetch` on form submit only) and the only raster images are the tiny logo SVGs plus the static OG image (which is never loaded by a visitor's browser — social platforms fetch it server-side when generating a link preview). This page should score close to trivially well; if that ever regresses, suspect a newly-added image or client-side library first.
